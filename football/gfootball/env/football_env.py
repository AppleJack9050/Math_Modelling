...
class FootballEnv(gym.Env):
  ...
  def step(self, action):
    ...
    _, reward, done, info, team1_score, team2_score = self._env.step(self._get_actions())
    score_reward = reward
    if self._agent:
    ...
    return (self.observation(), np.array(reward, dtype=np.float32), done, info, team1_score, team2_score)
